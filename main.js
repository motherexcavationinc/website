...
const renderMedia = (media, title) => {
  ...
  if (media.type === 'video') {
    return `<div class="media-slide"><video controls playsinline preload="none" aria-label="${escapeHtml(title)}" ${media.poster ? `poster="${mediaUrl(media.poster)}"` : ''} src="${mediaUrl(media.url)}"></video><a class="media-fallback" href="${mediaUrl(media.url)}" target="_blank" rel="noopener noreferrer">Open video ${externalArrow}</a></div>`;
  }
...
const projectMatchesService = (project, service) =>
  !service || (Array.isArray(project.services) && project.services.includes(service));

const renderEmptyWork = (list) => {
  list.innerHTML = '<p class="work-empty">No projects have been added yet.</p>';
};
...
const loadWork = async () => {
  ...
  try {
    ...
    workLists.forEach((list) => {
      const section = list.closest('[data-work-section]');
      ...
      const visibleProjects = projects
        ...
        .slice(0, limit > 0 ? limit : projects.length);
      if (!visibleProjects.length) {
        renderEmptyWork(list);
        return;
      }
    ...
    workLists.forEach(renderEmptyWork);
    console.error(error);
  }
};
